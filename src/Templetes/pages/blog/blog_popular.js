...
const Blog_popular = ()=>{
    ...
    axios.get(`https://tourismdatabase.herokuapp.com/api/blogs/${1}`)
    ...
    axios.get(`https://tourismdatabase.herokuapp.com/api/blogs/${2}`)
  ...
  return(
  <>
       <div className=" bg-light text-muted text-center">
       {Api.map(card=>(   <div key={card.title} className="row row-cols-md-2">
                 <div className="col-md-6">
                     <img src={card.image} className="img-fluid" alt={card.title}/>
                 </div>
                 <div className="col-md-6">
                     <h5 className="text-muted"><Link style={{color:"green" }} to={"/detailpage/"+card.id}>{card.title}</Link></h5>
             ...
             <li>{Api1.map(card=>(  <h6 className="border text-muted text-center text-left"><Link className="nav-link active rounded text-danger" to={"/detailpage/"+card.id}>{card.title}</Link></h6>  ))}</li>
             ...
             <li>{Api3.map(card=>(  <h6 className="border text-muted text-left"><Link className="nav-link active rounded text-danger" to={"/detailpage/"+card.id}>{card.title}</Link></h6>  ))}</li>
             </ul>
         </div>   
